...
void test_def_GPIO_as_Output(int a, int b) {                                          // avec  ( a >= 0  et a <= 31 )  et  ( 0 < a ou > 31 ) 
	...
	printf("readback direction_gpio_bank1 = %04x\n", direction_gpio_bank1);           // tous les bits � 0 sauf le a eme bit � 1 
	printf("\n");
...
void test_def_GPIO_as_Input(int a, int b) {                                           // avec  ( a >= 0  et a <= 31 )  et  ( 0 < a ou > 31 )
	...
	printf("readback direction_gpio_bank1  = %04x\n", direction_gpio_bank1);          // tous les bits � 1 sauf le a eme bit � 0
	printf("\n");
...
void test_assignment_Output_H(int a, int b){                                          // avec  ( a >= 0  et a <= 31 )  et  ( 0 < a ou > 31 )
	...
	printf("readback value_gpio_bank1 =  %04x\n", value_gpio_bank1);                  // tous les bits � 0 sauf le a eme bit � 1
	printf("\n");
...
void test_assignment_Output_L(int a) {                                               // avec  ( a >= 0  et a <= 31 )  
	...
	printf("readback value_gpio_bank1 =  %04x\n", value_gpio_bank1);                 // tous les bits � 1 sauf le a eme bit � 0
	printf("\n");
}
...
int main(int argc, char** argv){
  ...
  /*Test d�finition de la direcction GPIO comme sortie*/
  test_def_GPIO_as_Output(index0, index50);
  printf(" \n _____________________________________________________________________________________________\n");

  /*Test d�finition de la direction GPIO comme entree*/
  test_def_GPIO_as_Input(index0, index50);
  printf(" \n _____________________________________________________________________________________________\n");

  /* Test d'affectation d'une sortie � 1 */
  test_assignment_Output_H(index2, index50);
  printf(" \n _____________________________________________________________________________________________\n");

  /* Test d'affectation d'une sortie � 0 */
  test_assignment_Output_L(index1);
  printf(" \n _____________________________________________________________________________________________\n");

  /* Test de l'impossibilt� d'affectation d'une valeur = 1 � une entr�e */
